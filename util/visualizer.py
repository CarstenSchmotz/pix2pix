    ...
    for label, im_data in visuals.items():
        ...
        if label == 'real_A':  # Handle combined input separately
            if im.shape[2] == 4:  # Check if there are 4 channels
                ...
                # Save original RGB and Depth images separately
                original_rgb_name = '%s_%s_rgb_original.png' % (name, label)
                original_depth_name = '%s_%s_depth_original.png' % (name, label)
                original_rgb_path = os.path.join(image_dir, original_rgb_name)
                original_depth_path = os.path.join(image_dir, original_depth_name)
                util.save_image(rgb_numpy, original_rgb_path, aspect_ratio=aspect_ratio)
                util.save_image(depth_numpy_3ch, original_depth_path, aspect_ratio=aspect_ratio)
                
                ims.append(original_rgb_name)
                ims.append(original_depth_name)
                txts.append(label + '_rgb_original')
                txts.append(label + '_depth_original')
                links.append(original_rgb_name)
                links.append(original_depth_name)

                if use_wandb:
                    ims_dict[label + '_rgb_original'] = wandb.Image(rgb_numpy)
                    ims_dict[label + '_depth_original'] = wandb.Image(depth_numpy_3ch)
